...
public class Hazard implements Comparable<Hazard> {
	...
	private static final String TAG = Hazard.class.getSimpleName();

	public static List<Hazard> createHazardsFromIncidentJsonContents(
			String jsonContents) {
		assert jsonContents != null;

		List<Hazard> result = new LinkedList<Hazard>();
		...
		try {
			...
			for (int i = 0; i < features.length(); i++) {
				...
				Hazard hazard = new Hazard(featureJSONObject);
				result.add(hazard);
			}
	...
	private List<ArrangementElement> arrangementElements;
	...
	public List<ArrangementElement> getArrangementElements() {
	...
	private void parseArrangementElements() {
		...
		arrangementElements = new LinkedList<ArrangementElement>();
		...
		if (elementsJsonObjects != null) {
			for (int i = 0; i < elementsJsonObjects.length(); i++) {
				...
				if (elementJsonObject != null) {
					ArrangementElement element = new ArrangementElement(
							elementJsonObject);
		...
		if (periodsJsonObjects != null) {
			for (int i = 0; i < periodsJsonObjects.length(); i++) {
				...
				if (periodJsonObject != null) {
					Period period = new Period(periodJsonObject);
					periods.add(period);
				}
			}
		}
		...
		if (roadJsonObjects != null) {
			for (int j = 0; j < roadJsonObjects.length(); j++) {
				...
				if (roadJsonObject != null) {
					Road road = new Road(roadJsonObject);
					roads.add(road);
				}
			}
		}
	 ...
	 */
	public int compareTo(Hazard another) {
		int result = 0;
